    ...
    def categories(self):
        ...
        if not self._categories:
            for v in self._data.values():  # get set of sub/categories for TOC later
                self._categories.add(tuple(v["Category"]))
...
class Csv(Powers):
    ...
    def fields(self):
        ...
        if not self._fields:
            ...
            all_fields.pop("Save")  # remove Save for CSV
...
class Dot(Powers):
    ...
    def template(self):
        if not self._template:
            gen_powers = True if "powers" in self._stem else False
            ...
            if gen_powers:
                if "Role" in self._dependencies:
                    dot_frontmatter += dot_subgraph_role
                if "Skill" in self._dependencies:
                    dot_frontmatter += dot_subgraph_skill
                if "Level" in self._dependencies:
    ...
    def write(self):
        ...
        if self._dependencies:
            prereq_strings += [f"Prereq_{dep}" for dep in self._dependencies]
        for node, vals in self.readable_dict:
            if "Prereq_Level" in vals.keys():
                vals["Prereq_Level"] = f"Level_{vals['Prereq_Level']}"
            prereqs = {k: vals[k] for k in vals.keys() if k in prereq_strings}
            logging.debug(
                "Prereqs: ",
                {k: vals[k] for k in vals.keys() if k in prereq_strings},
            )
            children = []
            ...
            if children:
                edges += self.get_edges(node, children)
                try:
                    loners.remove(self.get_edges(node))
                except ValueError:
                    pass
            elif add_loners:
                loners += self.get_edges(node)
        dot_string = self.template % ";\n\t".join(edges)
        return dot_string
